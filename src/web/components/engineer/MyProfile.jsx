...
class MyProfile extends Component {
  ...
  componentDidMount(){
    ...
    const jwt = getJwt()
    this.getEngineers(process.env.REACT_APP_API_URL+'/api/v1/engineer/byUserId/'+jwt.userId)
  }
  ...
  handleDelete(){
    const jwt = getJwt()
    axios.delete(process.env.REACT_APP_API_URL+'/api/v1/engineer/'+this.state.engineer_id, { headers: { Authorization: `Bearer ${jwt.jwtToken}`, email: jwt.email, userid: jwt.userId }} )
    .then(res => {
  ...
  changeProfilePicture(e){
    ...
    axios.put(`${process.env.REACT_APP_API_URL}/api/v1/engineer/changeProfilPicture/${this.state.engineer_id}`, formData, config)
      .then( res=>{
        this.setState({
